...
class ReferenceV2Repository:
    ...
    async def update_saved_quiz_by_legacy_id(
        ...
        *,
        ...
        if not updates:
            return await self.get_saved_quiz_by_legacy_id(legacy_saved_quiz_id)
        updated = await self.saved_quizzes_collection.find_one_and_update(
    ...
    async def update_saved_quiz_by_id(
        self,
        saved_quiz_id: str,
        *,
        quiz_id: Optional[str] = None,
        ...
        if not updates:
            return await self.get_saved_quiz_by_id(saved_quiz_id)
        try:
            updated = await self.saved_quizzes_collection.find_one_and_update(
