...
class BillingHistoryResponse(BaseModel):
    invoices: list[BillingHistoryItemResponse] = Field(default_factory=list)
...
def _extract_current_period_end(subscription) -> str | None:
...
def _select_relevant_subscription(subscriptions) -> dict | None:
...
def _serialize_invoice(invoice) -> BillingHistoryItemResponse:
...
async def create_checkout_session(
    ...
    if (
        user.get("subscription_plan") == payload.plan
    ...
    session = stripe_client.checkout.Session.create(
        ...
        cancel_url=f"{settings.FRONTEND_BASE_URL}/profile?payment=cancelled",
