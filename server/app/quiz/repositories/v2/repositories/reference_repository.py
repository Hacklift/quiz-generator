...
class ReferenceV2Repository:
    ...
    async def get_folder_by_public_id(self, folder_id: str) -> FolderDocumentV2 | None:
        return await self.get_folder_by_id(folder_id) or await self.get_folder_by_legacy_id(folder_id)
    ...
    async def upsert_folder_item(
        self,
        folder_item: FolderItemDocumentV2,
        *,
        revive_deleted: bool = False,
    ) -> FolderItemDocumentV2:
        return await self.upsert_folder_item_by_legacy_id(
            folder_item,
    ...
    async def get_folder_item_by_public_id(self, folder_item_id: str) -> FolderItemDocumentV2 | None:
        return await self.get_folder_item_by_id(folder_item_id)
    ...
    async def get_saved_quiz_by_public_id(
        ...
        return await self.get_saved_quiz_by_id(
            saved_quiz_id,
            user_id=user_id,
        ) or await self.get_saved_quiz_by_legacy_id(
            saved_quiz_id,
            user_id=user_id,
        )
    ...
    async def get_quiz_history_by_public_id(
        ...
        return await self.get_quiz_history_by_id(history_id, user_id=user_id)
