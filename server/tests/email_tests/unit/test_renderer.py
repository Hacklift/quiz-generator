...
def test_render_quiz_link():
    ...
    desc = "D"
    link = "http://x"
    msg = render_email("quiz_link", "user@example.com", {"title": title, "description": desc, "link": link}, sender_email=SENDER_EMAIL)
...
def test_render_verification_contains_link_and_code():
    code = "999"
    token = "ttt"
    msg = render_email("verification", "user@example.com", {"code": code, "token": token}, sender_email=SENDER_EMAIL)
    assert msg["To"] == "user@example.com"
    assert msg["From"] == SENDER_EMAIL
    body = msg.get_payload()
    assert code in body
...
def test_render_password_reset_contains_link_and_code():
    code = "321"
    token = "tok"
    msg = render_email("password_reset", "user@example.com", {"code": code, "token": token}, sender_email=SENDER_EMAIL)
    assert msg["To"] == "user@example.com"
    assert msg["From"] == SENDER_EMAIL
    body = msg.get_payload()
    assert code in body
...
def test_render_custom_template():
    subj = "Hi"
    body_text = "Body here"
    msg = render_email("custom", "user@example.com", {"subject": subj, "body": body_text}, sender_email=SENDER_EMAIL)
...
def test_render_quiz_link_missing_vars_raises_keyerror():
    try:
        render_email("quiz_link", "user@example.com", {"description": "D", "link": "http://x"}, sender_email=SENDER_EMAIL)
    except KeyError:
        return
...
def test_renderer_unknown_template_returns_default_notification():
    msg = render_email("non_existent_template", "a@test.com", {}, sender_email=SENDER_EMAIL)
...
def test_renderer_uses_allowed_origins_for_links(monkeypatch):
    ...
    msg = renderer_module.render_email("verification", "user@example.com", {"code": "999", "token": "tok"}, sender_email=SENDER_EMAIL)
    body = msg.get_payload()
    assert "https://example.com/auth/verify-email/?token=tok" in body


def test_renderer_rejects_header_injection_in_untrusted_title():
    with pytest.raises(ValueError, match="Subject"):
        render_email(
            "quiz_link",
            "user@example.com",
            {"title": "Legitimate\r\nBcc: injected@example.com", "description": "D", "link": "http://x"},
            sender_email=SENDER_EMAIL,
        )


def test_renderer_rejects_header_injection_in_sender():
