...
export const getUserFolders = async (userId: string) => {
  const res = await axios.get(`${API_BASE}?user_id=${userId}`);
  return res.data;
};

export const createFolder = async ({ userId, name }: { userId: string; name: string }) => {
...
export const renameFolder = async (folderId: string, newName: string) => {
  const res = await axios.put(`${API_BASE}/${folderId}/rename`, {
    new_name: newName,
  });
  return res.data;
};
...
export const deleteFolder = async (folderId: string) => {
  const res = await axios.delete(`${API_BASE}/${folderId}/delete`);
  return res.data;
};
...
export const removeQuizFromFolder = async (folderId: string, quizId: string) => {
  const res = await axios.post(`${API_BASE}/${folderId}/remove_quiz`, {
    quiz_id: quizId,
  });
  return res.data;
};
...
export const getFolderById = async (folderId: string) => {
  const res = await axios.get(`${API_BASE}/${folderId}`);
  return res.data;
};
